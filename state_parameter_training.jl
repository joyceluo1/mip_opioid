...
using DifferentialEquations, Flux, Optim, DiffEqFlux, Plots, CSV, DataFrames, DelimitedFiles, Sundials, Tables
...
for i in 1:length(states)
    ...
    function plotFit(param, u0, st)
        ...
        pl=plot(sol_fit, vars=[2 3 4 5 6], lw=2, legend=:outertopright, label = ["P" "I" "A" "R" "D"], dpi=1200)
    ...
    function train_model()
        ...
        # Train the ODE
        resinit=DiffEqFlux.sciml_train(loss_adjoint,pguess,ADAM(0.0001), cb=callback, maxiters=20000)
        res = DiffEqFlux.sciml_train(loss_adjoint,resinit.minimizer,BFGS(initial_stepnorm = 1e-5), cb=callback)
        println("Fitted parameters:")
        println("$((res.minimizer).^2)")
        return(res)
    end
    ...
    ls = plot(losses, xlabel = "Iterations", ylabel = "Loss", yaxis = :log, legend = false, dpi=1200)
