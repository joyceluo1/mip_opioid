...
using JuMP and Gurobi. We allow facilities to open and close in this model.
...
for state in states
    ...
    # Define cost of opening and closing (opening > closing) - scaled down
    c_o = 0.1
    c_c = 0.02
    ...
    # set_optimizer_attribute(model_nc, "MIPGap", 0)
    ...
    # Define objective
    @expression(model_nc, equity, lambda_pr*sum(pr_weight.*x) + lambda_svi*sum(SVI_weight.*x))

    @objective(model_nc, Min, lambda_D*D[T] + lambda_A*sum(A) + equity + c_o*sum(u) + c_c*sum(v))
    # Define constraints
    @constraint(model_nc, [i = 1:len], sum(x) <= N)
    @constraint(model_nc, [i = 1:len], u[i] >= x[i] - n[i])
    @constraint(model_nc, [i = 1:len], v[i] >= n[i] - x[i])
    @constraint(model_nc, [i = 1:len], x[i]*z[i] == 1)
    @constraint(model_nc, [t = 1:T], sum(d_bar[:, t]*100000) <= d_lim[t])
    @expression(model_nc, expr[t = 1:T], sum(d_inv*(d_bar[i,t]*100000 - n[i]*z[i]*d_bar[i,t]*100000) for i in 1:len))
    @constraint(model_nc, [i = 1:len, t = 1:T], d_bar[i, t]*100000 - (county_pops[i]/sum(county_pops))*d_lim[t] ≤ push*(county_pops[i]/sum(county_pops))*d_lim[t])
    @constraint(model_nc, [i = 1:len, t = 1:T], -(d_bar[i, t]*100000 - (county_pops[i]/sum(county_pops))*d_lim[t]) ≤ push*(county_pops[i]/sum(county_pops))*d_lim[t])
    ...
    println("Equity part: ", value(equity))
    println("Solve time: ", solve_time(model_nc))
    S_pred = []
    P_pred = []
    I_pred = []
    A_pred = []
    ...
    CSV.write("open_close_experiments/" * state * "_x_dist.csv", x_dist, writeheader=true, header=["FIPS", "x"])
    for i in 1:T
        println(i)
        for j in 1:len
    ...
    bud_dist = Dict()
    for j in 1:len
        bud_dist[model_data[j, "FIPS"]] = round(value(d_bar[j, 1])*100000, digits = 2)
    end
    CSV.write("open_close_experiments/" * state * "_bud_dist.csv", bud_dist,  writeheader=true, header=["FIPS", "bud"])
    vals = []
...
CSV.write("open_close_experiments/final_sols.csv", Tables.table(cat), writeheader = true, header = states)
CSV.write("open_close_experiments/model_msrs_new.csv", Tables.table(msrs), writeheader = true, header = states)
CSV.write("open_close_experiments/zeta_over_time.csv", Tables.table(zeta_time), writeheader = true, header = states)