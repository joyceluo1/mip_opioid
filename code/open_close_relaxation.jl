...
location and budget allocation optimization problem for set of US states using JuMP and 
Gurobi. We allow facilities to open and close in this model.
...
#        "WI"]
states = ["DE", "ME", "IN", "FL", "NY", "CA"]

global cat = Array{Float64}(undef, 3, 0)
global msrs = Array{Float64}(undef, 3, 0)
...
for state in states
    ...
    # Define cost of opening and closing (opening > closing) 
    c_o = 0.1
    c_c = 0.02
    ...
    uv_df = DataFrame(u = value.(u), v = value.(v))
    CSV.write("open_close_relax/" * state * "_uv.csv", uv_df)
    ...
    CSV.write("open_close_relax/" * state * "_x_dist.csv", x_dist, writeheader=true, header=["FIPS", "x"])
    for i in 1:T
        println(i)
        for j in 1:len
    ...
    CSV.write("open_close_relax/" * state * "_bud_dist.csv", bud_df)
    vals = []
...
CSV.write("open_close_relax/final_sols.csv", Tables.table(cat), writeheader = true, header = states)
CSV.write("open_close_relax/model_msrs_new.csv", Tables.table(msrs), writeheader = true, header = states)
CSV.write("open_close_relax/zeta_over_time.csv", Tables.table(zeta_time), writeheader = true, header = states)