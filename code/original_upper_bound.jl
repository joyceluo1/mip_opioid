...
for state in states
    ...
    model_msrs = DataFrame(CSV.File("mccormick_cont_relax/model_msrs_new.csv"))
    opt_xs = DataFrame(CSV.File("mccormick_cont_relax/" * state * "_x_dist.csv"))
    opt_xs = sort!(opt_xs, [:FIPS])
    opt_buds = DataFrame(CSV.File("mccormick_cont_relax/" * state * "_bud_dist.csv"))
...
CSV.write("mccormick_cont_relax/obj_UBs.csv", Tables.table(obj_vals), writeheader = true, header = states)
CSV.write("mccormick_cont_relax/final_UB_sols.csv", Tables.table(final_sols), writeheader = true, header = states)