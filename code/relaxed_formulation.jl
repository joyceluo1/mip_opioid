...
using JuMP, Gurobi, LinearAlgebra, CSV, DataFrames, Plots, DelimitedFiles, ArgParse, Tables, MathOptInterface, DataStructures
...
for state in states
    ...
    @constraint(model_nc, [i = 1:len, t = 1:T], r[i,t] >= 0.05)
    ...
    println(N)
    CSV.write("rev2_orig_relaxation/" * state * "_x_dist.csv", x_dist, writeheader=true, header=["FIPS", "x"])
    # for i in 1:T
    #     println(i)
    #     for j in 1:len
    ...
    println(d_lim[1])
    CSV.write("rev2_orig_relaxation/" * state * "_bud_dist.csv", bud_df)
    vals = []
    ...
    # push!(vals, sum(x_dist.vals) >= N)
    # push!(vals, (d_lim[1] + 1 >= sum(bud_df[!, "1"])) && (sum(bud_df[!, "1"]) >= d_lim[1] - 1))
...
CSV.write("rev2_orig_relaxation/final_sols.csv", Tables.table(cat), writeheader = true, header = states)
CSV.write("rev2_orig_relaxation/model_msrs_new.csv", Tables.table(msrs), writeheader = true, header = states)
