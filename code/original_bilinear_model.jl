...
for state in states
    ...
    @objective(model_nc, Min, lambda_D*D[T] + lambda_A*sum(A) + equity + inf*h)
    ...
    CSV.write("normalized_model/" * state * "_x_dist.csv", x_dist, writeheader=true, header=["FIPS", "x"])
    # for i in 1:T
    #     println(i)
    #     for j in 1:len
    ...
    CSV.write("normalized_model/" * state * "_bud_dist.csv", bud_df)
    vals = []
...
CSV.write("normalized_model/final_sols.csv", Tables.table(cat), writeheader = true, header = states)
CSV.write("normalized_model/model_msrs_new.csv", Tables.table(msrs), writeheader = true, header = states)
CSV.write("normalized_model/zeta_over_time.csv", Tables.table(zeta_time), writeheader = true, header = states)